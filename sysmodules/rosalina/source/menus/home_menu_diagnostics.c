...
static bool ValidateResidentLauncher(const u8 *resident, u16 *usedOut)
{
    /* HOME keeps the Launcher object beginning at file offset +8, without
       Launcher.dat's header. Reconstruct that header before applying the
       documented file-layout validator. */
    memset(g_launcherRaw, 0, CTH_LAUNCHER_SIZE);
    g_launcherRaw[0] = 4;
    memcpy(g_launcherRaw + 8, resident, CTH_LAUNCHER_SIZE - 8);
    if (g_launcherRaw[CTH_HOME_ROWS_OFFSET] > 5)
        return false;
    return ValidateLayout(g_launcherRaw, true, usedOut);
}
...
static u32 TraversalKey(s16 position, s16 origin, u32 rows, bool rowMajor)
{
    ...
    u32 columns = VisibleHomeColumns(rows);
    if (columns == 0) return relative;
                                    ...
                                    u32 rows, bool rowMajor)
{
    ...
    /* Treat each visible screen as a fixed grid. Fill every row of the
       current screen left-to-right before advancing to the next screen.
       A partial last screen keeps the same width; it must not be narrowed. */
    u32 columns = VisibleHomeColumns(rows);
    if (columns == 0) return (s16)(origin + rank);
    u32 screenCapacity = rows * columns;
    u32 screen = rank / screenCapacity;
    u32 within = rank % screenCapacity;
    u32 row = within / columns;
    u32 column = within % columns;
    return (s16)(origin + screen * screenCapacity + column * rows + row);
}
            ...
            if (R_SUCCEEDED(map))
            {
                     ...
                     offset += 8)
                {
                    ...
                    if (!quickMatch)
                        continue;
                    u16 used = 0;
                    if (ValidateResidentLauncher(bytes + offset, &used))
                    {
    ...
    *algorithmOut = header->algorithm;
        ...
        (header->algorithm == 1 || header->algorithm == 2))
    {
        ...
        SortFolderIndexes(folderIds, folderCount, g_launcherRaw,
                          header->algorithm == 2);
        g_livePlannedFolderCount = folderCount;
        g_livePlannedFoldersFirst = foldersFirst;
        if (R_SUCCEEDED(res) &&
        ...
        if (R_SUCCEEDED(res))
        {
            ...
            u32 launcherObjectCount = topLauncherCount + folderCount;
            if (launcherObjectCount > CTH_LAYOUT_SLOTS ||
            ...
            for (u32 i = 0; R_SUCCEEDED(res) && i < folderCount; i++)
            {
                ...
                if (position < 0 || position >= CTH_LAYOUT_SLOTS ||
                    launcherOccupied[position])
                    res = (Result)-126;
                else
                    launcherOccupied[position] = true;
                memcpy(g_launcherRaw + CTH_FOLDER_POSITION_OFFSET +
...
static const char *InventoryNameForTitle(u64 titleId, char *name, u32 capacity)
{
    name[0] = 0;
    CthRequestHeader *header = (CthRequestHeader *)g_sortRequest;
    if (header->magic != CTH_REQUEST_MAGIC || header->entryCount > 900)
        return "none";
    CthRequestEntry *entries = (CthRequestEntry *)(header + 1);
    for (u32 i = 0; i < header->entryCount; i++)
        if (entries[i].titleId == titleId && (entries[i].flags & 1))
        {
            RequestTitleToAscii(&entries[i], name, capacity);
            return "catalog";
        }
    return "none";
}

static void WriteVisibleObjectInventory(Handle home, u32 records,
                                        volatile s16 *inlineMap,
                                        u32 indirectMap)
{
    const u32 recordsLocal = 0x00D00000;
    const u32 indirectLocal = 0x00F00000;
    const u32 recordsSize = 0x3B000;
    ...
    int length = sprintf(g_objectInventory,
        ...
        records, recordResult, indirectMap, indirectResult);
    for (u32 coordinate = 0; coordinate < CTH_LAYOUT_SLOTS &&
         length < (int)sizeof(g_objectInventory) - 384; coordinate++)
    {
        s16 inlineRecord = inlineMap[coordinate];
        s16 indirectRecord = R_SUCCEEDED(indirectResult) ?
            *(volatile s16 *)(indirectLocal + indirectOffset + coordinate * 2) : -1;
        s16 record = inlineRecord >= 0 && inlineRecord < 420 ?
                     inlineRecord : indirectRecord;
        ...
        if (LooksLikeTitleId(titleId))
            for (u32 slot = 0; slot < CTH_LAYOUT_SLOTS; slot++)
            {
                ...
                {
                    sdSlot = (s32)slot;
                    sdPosition = ReadS16(g_sortRaw, 0xCB0 + slot * 2);
                    sdFolder = *(s8 *)(g_sortRaw + 0xF80 + slot);
                }
                if (ReadU64(g_launcherRaw, 8 + slot * 8) == titleId)
                {
                    launcherSlot = (s32)slot;
                    launcherPosition = ReadS16(g_launcherRaw, 0xD9A + slot * 2);
        ...
        length += sprintf(g_objectInventory + length,
            "%lu,%d,%d,%08lx%08lx,%08lx:%08lx:%08lx:%08lx:%08lx:%08lx,"
            "%08lx,%ld,%d,%d,%ld,%d,%d,%s,%s\n",
            (unsigned long)coordinate, inlineRecord, indirectRecord,
            words[1], words[0], words[2], words[3], words[4], words[5],
            words[6], words[7], words[14], (long)sdSlot, sdPosition,
    ...
    {
        u64 written = 0;
        IFile_Write(&file, &written, g_objectInventory, length, FS_WRITE_FLUSH);
        IFile_SetSize(&file, length);
        IFile_Close(&file);
    }
}
...
static u32 ScanLiveIconClassV010Rc8(Handle home)
{
    ...
    if (iconModel != 0)
    {
        ...
        if (R_SUCCEEDED(map))
        {
            ...
            WriteVisibleObjectInventory(home, records, inlineMap, indirectMap);
            ...
            if (g_livePlannedFolderCount > 0)
            {
                ...
                for (u32 i = 0; i < topLevelRecordCount; i++)
            ...
            {
                s16 candidate = inlineMap[i];
                if (candidate < 0) continue;
                bool title = false, seen = false;
                for (u32 d = 0; d < desiredCount; d++)
                    if (candidate == (s16)g_liveDesiredRecords[d]) title = true;
                for (u32 prior = 0; prior < i; prior++)
                    if (inlineMap[prior] == candidate) seen = true;
                if (!title && !seen)
                {
                    u32 occurrences = 0;
                    for (u32 j = 0; j < 420; j++)
                        if (inlineMap[j] == candidate) occurrences++;
                    length += sprintf(report + length, "%d@%lu(x%lu),",
                        candidate, (unsigned long)i, (unsigned long)occurrences);
                    if (R_SUCCEEDED(unmatchedMapResult) && candidate < 420)
                    {
                        const u32 *w = (const u32 *)(unmatchedLocal +
                            unmatchedRecordOffset + candidate * 0x230);
                        length += sprintf(report + length,
                            "{id=%08lx%08lx w2=%08lx w3=%08lx w4=%08lx "
                            "w5=%08lx w6=%08lx w7=%08lx w14=%08lx}",
                            w[1], w[0], w[2], w[3], w[4], w[5], w[6],
                            w[7], w[14]);
                    }
                    nonTitleInline++;
                }
            ...
            if (R_SUCCEEDED(unmatchedMapResult))
                svcUnmapProcessMemoryEx(CUR_PROCESS_HANDLE, unmatchedLocal,
                                        unmatchedMapSize);
            length += sprintf(report + length, " count=%lu\n",
                (unsigned long)nonTitleInline);
            ...
            length += sprintf(report + length,
                ...
                "live_update_partial=%lu\nlive_update_deferred=0\n",
                positionCount, orderedCount,
        ...
        if (R_SUCCEEDED(map))
        {
            ...
            if (R_SUCCEEDED(map))
            {
                ...
                if (g_livePlannedFolderCount > 0)
                {
                    ...
                    u32 occurrences = 0, isTitle = 0, isFolderMember = 0;
                    for (u32 i = 0; i < 420; i++)
                        if (indices[i] == candidate) occurrences++;
                    for (u32 i = 0; i < topLevelRecordCount; i++)
                        if (candidate == (s16)g_liveTopLevelRecords[i]) isTitle = 1;
                    for (u32 i = 0; i < folderRecordCount; i++)
                        if (candidate == (s16)g_liveFolderRecords[i]) isFolderMember = 1;
                    length += sprintf(report + length,
                        "folder_probe_indirect old=%d new=%d record=%d occurrences=%lu "
                ...
                for (u32 i = 0; i < 420 && nonTitleIndirect < 48; i++)
                {
                    s16 candidate = indices[i];
                    if (candidate < 0) continue;
                    bool title = false, seen = false;
                    for (u32 d = 0; d < desiredCount; d++)
                        if (candidate == (s16)g_liveDesiredRecords[d]) title = true;
                    for (u32 prior = 0; prior < i; prior++)
                        if (indices[prior] == candidate) seen = true;
                    if (!title && !seen)
                    {
                        u32 occurrences = 0;
                        for (u32 j = 0; j < 420; j++)
                            if (indices[j] == candidate) occurrences++;
                        length += sprintf(report + length, "%d@%lu(x%lu),",
                            candidate, (unsigned long)i, (unsigned long)occurrences);
                        nonTitleIndirect++;
                    }
                ...
                length += sprintf(report + length, " count=%lu\n",
                    (unsigned long)nonTitleIndirect);
                for (u32 i = 0; i < 60; i += 10)
                    length += sprintf(report + length,
...
Result CthulhuHomeMenu_CaptureObjectInventory(void)
{
    ...
    if (R_FAILED(res)) return res;
    res = svcControlProcess(home, PROCESSOP_SCHEDULE_THREADS, 1, 0);
    ...
    u32 launcherAddress = 0, launcherMatches = 0;
    MemInfo launcherMem = {0};
    if (R_SUCCEEDED(res) && R_FAILED(launcherFileResult))
    {
        launcherResidentResult = DiscoverLauncherRuntime(home,
            CTH_NAND_RAW_ADDRESS, &launcherAddress, &launcherMem,
            &launcherMatches);
        if (R_SUCCEEDED(launcherResidentResult))
        {
            launcherResidentResult = svcMapProcessMemoryEx(CUR_PROCESS_HANDLE,
                launcherMem.base_addr, home, launcherMem.base_addr,
                launcherMem.size, 0);
            if (R_SUCCEEDED(launcherResidentResult))
            {
                memset(g_launcherRaw, 0, CTH_LAUNCHER_SIZE);
                ...
                memcpy(g_launcherRaw + 8, (const void *)launcherAddress,
                       CTH_LAUNCHER_SIZE - 8);
                svcUnmapProcessMemoryEx(CUR_PROCESS_HANDLE,
                    launcherMem.base_addr, launcherMem.size);
            }
        }
        res = launcherResidentResult;
    }
    else if (R_SUCCEEDED(res))
        res = launcherFileResult;
    u16 launcherUsed = 0;
    if (R_SUCCEEDED(res) && !ValidateLayout(g_launcherRaw, true, &launcherUsed))
        res = (Result)-122;
    if (R_SUCCEEDED(res))
        for (u32 slot = 0; slot < CTH_LAYOUT_SLOTS; slot++)
        {
            u64 titleId = ReadU64(g_launcherRaw, 8 + slot * 8);
            s16 position = ReadS16(g_launcherRaw, 0xD9A + slot * 2);
            s8 folder = *(s8 *)(g_launcherRaw + 0x106A + slot);
            if (LooksLikeTitleId(titleId) && folder == -1 && position >= 0 &&
                position < CTH_LAYOUT_SLOTS &&
                g_sortGrid[position] == UINT64_MAX)
                g_sortGrid[position] = titleId;
        }
    if (R_SUCCEEDED(res))
    ...
    {
        u64 written = 0;
        IFile_Write(&captureFile, &written, captureReport, captureLength,
                    FS_WRITE_FLUSH);
        IFile_SetSize(&captureFile, captureLength);
        IFile_Close(&captureFile);
    }
    return res;
}
