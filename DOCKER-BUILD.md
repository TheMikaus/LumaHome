...
## Persistent container for repeated builds

The commands above start a fresh container per build. When you are iterating —
edit, build, read the errors, edit again — `compose.yaml` is quicker. It defines
one service, `devkitarm`, built from the same `Dockerfile` and tagged with the
same `lumahome-toolchain:1` image, that stays running so each build reuses it:

```text
docker compose up -d
docker compose exec devkitarm make clean
docker compose exec devkitarm make -j2
```

Same Dockerfile, same image, same result: this path and `build_docker.ps1`
produce a byte-identical `boot.firm`. It differs only in mounting the checkout
at `/workspace` rather than `/project`, which the Makefiles do not care about,
and in setting `safe.directory` through the environment so that git works on the
bind-mounted checkout — the Makefiles shell out to `git describe` to stamp
`REVISION`, `VERSION_MAJOR/MINOR/BUILD` and `COMMIT`.

Two notes:

- **In a shallow clone, `git describe` finds no tag**, so the Makefiles fall
  back to `v0.0.0-0`, and `arm9/Makefile` additionally infers `IS_RELEASE=1`.
  The build still succeeds, but the payload self-reports as *release 0.0.0*,
  which defeats step 5 of "Locate and deploy the result safely" above. Run
  `git fetch --unshallow` on the host to fix it; no container change helps.
- After editing the `Dockerfile`, run `docker compose build` and
  `docker compose up -d` to recreate the container from the new image.

Stop the idle container with `docker compose stop` when you are done.
